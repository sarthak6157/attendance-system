"""Session routes — stores GPS coords when session starts."""
import secrets
...
from schemas.schemas import SessionCreate, SessionListOut, SessionOut
...
def create_session(payload: SessionCreate, current_user: User = Depends(FacultyOrAdmin), db: DBSession = Depends(get_db)):
    s = Session(
        course_id=payload.course_id, faculty_id=current_user.id,
        title=payload.title or "Lecture", location=payload.location,
        scheduled_at=payload.scheduled_at, grace_minutes=payload.grace_minutes,
...
def list_sessions(
    course_id: Optional[int] = None, faculty_id: Optional[int] = None,
...
def get_active(_: User = Depends(get_current_user), db: DBSession = Depends(get_db)):
...
def start_session(
    ...
    current_user: User = Depends(FacultyOrAdmin), db: DBSession = Depends(get_db),
):
    s = db.query(Session).filter(Session.id == session_id).first()
    ...
    if s.status != SessionStatus.scheduled:
        raise HTTPException(status_code=400, detail=f"Session already {s.status.value}.")
    s.status     = SessionStatus.active
    s.started_at = datetime.utcnow()
    s.qr_token   = secrets.token_urlsafe(16)
    # Save GPS of classroom at start time
    if gps_lat: s.gps_lat = gps_lat
    if gps_lng: s.gps_lng = gps_lng
