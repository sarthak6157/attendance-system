"""Session routes: create, list, start, end, QR."""
import secrets
...
def create_session(payload: SessionCreate, current_user: User = Depends(FacultyOrAdmin), db: DBSession = Depends(get_db)):
    s = Session(
        ...
        title=payload.title or "Lecture",
...
def get_active_sessions(_: User = Depends(get_current_user), db: DBSession = Depends(get_db)):
...
def start_session(session_id: int, current_user: User = Depends(FacultyOrAdmin), db: DBSession = Depends(get_db)):
    s = db.query(Session).filter(Session.id == session_id).first()
    if not s:
